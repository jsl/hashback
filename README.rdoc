...
HashBack is a simple Object-hash mapping (OHM) system for Ruby.  It allows for serializable classes to be saved, retrieved and  
deleted from any key-value store supported by Moneta (a unified interface to key-value stores).
...
to use by passing an appropriate class name to HashBack::Resource.setup.  See the Moneta documentation available backends.
...
* A system for associating objects, perhaps constrained to objects that have a 1 - 1 mapping (since it's) not 
